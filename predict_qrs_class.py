...
def predict_qrs_class(qrs):
    """
    Predict qrs class
    :param qrs: list with qrs values. Has to be length of 108
    :return: 'N' or 'V' if qrs is normal or abnormal
    """
    assert len(qrs) == 108, 'Qrs must be length of 108'

    qrs_surface = np.sum(np.abs(qrs))
